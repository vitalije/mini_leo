  ...
  */
  ...
  fn pyundo(_py: Python, tid:usize, data:&str) {
    TREES.lock().unwrap().get_mut(&tid).map(|t|{
      if data.starts_with("ip:") {
        undo_insert_parts(&mut t.outline, data);
      } else if data.starts_with("db:") {
        undo_delete_blocks(&mut t.outline, data);
      }
    });
  }
  #[pyfn(m)]
  ...
  fn pyredo(_py: Python, tid:usize, data:&str) {
    TREES.lock().unwrap().get_mut(&tid).map(|t|{
      if data.starts_with("ip:") {
        redo_insert_parts(&mut t.outline, &data);
      }
    });
  }
  #[pyfn(m)]
